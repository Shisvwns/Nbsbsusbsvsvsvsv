...
local listfastattack = {'Normal Attack','Fast Attack','Super Fast Attack'}
    ...
    spawn(function()
        ...
        local ToggleCandy = Tabs.Main:AddToggle("ToggleCandy", {Title = "Auto Candy Event New 😂😍", Default = false })
        ...
        spawn(function()
            while wait() do 
                if _G.AutoCandy then
                    pcall(function()
                        toTarget(CFrame.new(-16603.197265625, 130.3873748779297, 1087.16455078125))
...
local boss = Tabs.Main:AddSection("Boss Farm")
    ...
    if First_Sea then
		tableBoss = {"DauCoGhe Raid Boss [Lv. 10000]","The Gorilla King","Bobby","Yeti","Mob Leader","Vice Admiral","Warden","Chief Warden","Swan","Magma Admiral","Fishman Lord","Wysper","Thunder God","Cyborg","Saber Expert"}
	elseif Second_Sea then
		tableBoss = {"DauCoGhe Raid Boss [Lv. 10000]","Diamond","Jeremy","Fajita","Don Swan","Smoke Admiral","Cursed Captain","Darkbeard","Order","Awakened Ice Admiral","Tide Keeper"}
	elseif Third_Sea then
		tableBoss = {"DauCoGhe Raid Boss [Lv. 10000]","Stone","Island Empress","Kilo Admiral","Captain Elephant","Beautiful Pirate","rip_indra True Form","Longma","Soul Reaper","Cake Queen"}
	end
                ...
                pcall(function()
                    ...
                    elseif game.ReplicatedStorage:FindFirstChild(_G.SelectBoss) then
						if ((game.ReplicatedStorage:FindFirstChild(_G.SelectBoss).HumanoidRootPart.CFrame).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 1500 then
							toTarget(game.ReplicatedStorage:FindFirstChild(_G.SelectBoss).HumanoidRootPart.CFrame)
					    end
                    end
                end)
            end
